...
| 기능 | 설명 |
| --- | --- |
| 변경 범위 분석 | 명시한 Git commit은 정확 범위로 고정하고, 그 외에는 diff·수정일·수동 변경 파일과 사용자 요약을 조합해 날짜·파일 누락을 보완합니다. |
...
| 근거 기반 테스트 시나리오 | 정상 케이스를 포함한 독립 시나리오를 약 3개 구성하고, 실패·경계 케이스는 실제 소스 분기가 있을 때만 추가합니다. |
| 안전한 결과 검증 | JSON 구조, 실제 테스트 데이터, 시나리오별 절차와 예상결과를 검증하고 표현 차이는 안전한 범위에서 자동 정리합니다. |
...
2. Git Diff, 시작일과 종료일, 수동 변경 파일 목록과 개발자 변경 요약을 조합해 변경 범위를 만듭니다.
   GitHub·GitLab commit URL, `commit:`·`SHA:` 표기 또는 SHA 단독행을 적으면 날짜와 현재
   작업트리보다 해당 commit을 우선하고, 선택한 프로젝트 폴더 안에서 그 commit이 실제로 바꾼
   파일 전체를 확정 범위로 사용합니다. 짧은 SHA도 단독행이면 사용할 수 있지만, 숫자로만
   이루어진 짧은 SHA는 업무번호와 구분하도록 `commit: 1234567`처럼 접두어를 붙입니다.
3. 의뢰서 또는 변경 로직 설명을 입력합니다.
4. 변경 파일과 언어별 import, include, endpoint, 호출 및 데이터 객체 관계를 따라 관련 소스를 선별합니다.
   명시 commit이 없는 불확실한 입력에서만 날짜 밖 후보를 살피며, 실제 Git 변경과 직접 참조,
   가까운 변경 시점 및 변경 hunk 근거가 함께 확인된 경우에만 변경 범위로 복구합니다.
...
명시한 commit URL이나 SHA가 있으면 날짜를 잘못 입력했더라도 commit을 우선하되, 범위 밖이라는
사실은 로그로 안내합니다. 존재하지 않는 SHA는 비슷한 commit으로 추측하지 않고 즉시 알립니다.
이때 소스 본문도 현재 checkout이 아닌 선택 commit의 snapshot을 사용하며, 민감 파일과 민감
파일에서 일반 이름으로 바뀐 rename은 경로 메타데이터만 남깁니다.

Git 저장소가 아니거나 `Git 변경 포함`을 끈 경우에는 선택한 날짜 범위의 파일 수정일과 수동
변경 경로를 사용합니다. 이 모드에서도 소스 본문과 참조 관계 분석은 동일하게 동작하지만,
파일 수정일은 실제 코드 변경 시점과 다를 수 있습니다. 삭제된 파일은 파일 시스템에 남지 않으므로
`삭제: src/legacy.py`처럼 변경 입력에 직접 적어야 합니다. 날짜와 수동 경로가 모두 없으면
추측으로 프로젝트 전체를 포함하지 않고 변경 근거 부족 오류를 표시합니다.
...
- 환경설정의 `문서 시스템명`을 비워두면 의뢰서에 명시된 시스템명, 근거가 확인된 AI 응답,
  프로젝트 루트 순으로 자동 판정합니다. 직접 입력한 경우에만 다운로드 파일명, 구조화 문서
  제목과 Excel 프로그램 정보의 `구분`에 고정 적용합니다.
...
- 응답 문자열 안의 비정상적인 실제 줄바꿈·탭은 JSON 의미를 바꾸지 않는 범위에서 먼저 복구하고,
  그 밖의 JSON 문서 계약 오류만 기본 3회까지 교정 요청합니다.
...
- [변경 범위 회수 시뮬레이션](docs/SCOPE_SIMULATION.md)
